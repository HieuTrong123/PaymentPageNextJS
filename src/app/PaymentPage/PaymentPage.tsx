...
import dataRef from "@/components/Config/config.jsx";
...
export default function PaymentPage() {
  ...
  const showSuccessAlert = (text: string, status = "success", callback) => {
    Swal.fire({
      title: "Thông báo",
      text: text,
      icon: status,
  ...
  const brandName = (document.URL.match(/https:\/\/donhang\.(\w+?)\./) ||
    [])[1];
  const ref = brandName || queryParams.get("ref");
  const brand =
    ref && Object.keys(dataRef).includes(ref)
      ? dataRef[ref]
      : dataRef["default"];
  ...
  if (brand.favicon)
    document.querySelector('link[rel="icon"]').href = brand.favicon; // sửa favicon

  if (brand.sologan)
    document.querySelector('meta[name="description"]').content = brand.sologan;
  ...
  useEffect(() => {
    async function fetchOrder() {
      try {
        feting = true;
        const data =
          window.data || (await getOrder(orderHash).then((r) => r.data));
        window.data = data;
        setDetailsInfo(data);
    ...
    const checkCondition = async () => {
      ...
      if (
        ["đã gửi", "hoàn thành"].includes(
          detailsInfo?.status.text.toLocaleLowerCase()
        )
      ) {
        setIsStop(true);
        return;
  ...
  const depositRatio = Math.round(
    (deposit / totalMoneyAfterVATorDiscount) * 100
  );
  ...
  const employeeName = [customer.assign.last_name, customer.assign.first_name]
    .join(" ")
    .trim();
        ...
        <div>
          ...
          <div className={classes.info}>
            <ul className={`${classes.listInfo} ${classes.container}`}>
              ...
              <InfoITem
                title="Khách hàng"
                content={detailsInfo.customer.name}
              />
              <InfoITem
                title="Số điện thoại"
                content={detailsInfo.customer.phone}
              />
              <InfoITem
                title="Địa chỉ"
                content={detailsInfo.customer.address}
              />
            </ul>
            <ul className={`${classes.listInfo} ${classes.container}`}>
              ...
              <div className={classes.contact_img}>
                <Image
                  ...
                  height={288}
                />
                <Image src={phone_icon} alt="error" width={372} height={288} />
                <Image src={fb_icon} alt="error" width={372} height={288} />
              </div>
            </ul>

            {/* <div className={classes.box_img}>
                            <Image className={classes.img} src={ThienTrang} />
                        </div> */}
            {/* {totalPay > 0 && (
      ...
      <div className={`${classes.qrpay}`}>
        {totalPay > 0 ? (
          <ReactCardFlip flipDirection="horizontal" isFlipped={statePayment}>
            <div
              id="QR"
              className={`${classes.container} ${classes.height_left}`}
            >
            ...
            <div
              className={`${classes.container} ${classes.paymentMT} ${classes.height_left}`}
            >
              <div className={classes.box_flex}>
                <Image src={iconCheck} alt="error" width={372} height={288} />
              ...
              <div className={classes.box_flex}>
                ...
                <Image src={iconSupport} />
              </div>
              <div className={classes.box_flex}>
                <Image src={iconPayment} alt="error" width={372} height={288} />
                <p>
          ...
          <ReactCardFlip flipDirection="horizontal" isFlipped={stateTks}>
            <div
              id="QR"
              className={`${classes.container} ${classes.height_left} ${classes.tks}`}
            >
              <div className={classes.Payhead}>
                <Image
                  src={imgTks}
                  className={classes.imgTks}
                  alt="error"
                  width={372}
                  height={288}
                />
            ...
            >
              <div className={classes.box_flex}>
                <Image src={iconProduct} alt="error" width={372} height={288} />
                <p>
              ...
              <div className={classes.box_flex}>
                <Image src={iconSupport} alt="error" width={372} height={288} />
                <p>
        ...
        <div className={`${classes.container} ${classes.transport}`}>
          ...
          <p>Mã vận chuyển : {ships.track || "Chưa có thông tin"}</p>
          <p>
          ...
          <p>
            Phí vận chuyển :{" "}
            {ships.freeShip ? "Miễn phí" : "Khách hàng thanh toán phí ship"}
          </p>
        </div>
      </div>
    </div>
